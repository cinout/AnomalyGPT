...
for obj in objs:
    ...
    for i in range(len(prompt_state)):
        ...
        prompted_sentence = data.load_and_transform_text(
            prompted_sentence, torch.cuda.current_device()
        )
...
def encode_text_with_prompt_ensemble(model, obj, device):
    ...
    class_embeddings_normal = model({ModalityType.TEXT: normal_sentences})[
        ModalityType.TEXT
    ][0]
    class_embeddings_abnormal = model({ModalityType.TEXT: abnormal_sentences})[
        ModalityType.TEXT
    ][0]
    ...
    class_embeddings_normal = class_embeddings_normal / class_embeddings_normal.norm(
        dim=-1, keepdim=True
    )
    ...
    class_embeddings_abnormal = (
        class_embeddings_abnormal / class_embeddings_abnormal.norm(dim=-1, keepdim=True)
    )

    text_features = torch.cat(
        [class_embeddings_normal, class_embeddings_abnormal], dim=1
    )
...
class OpenLLAMAPEFTModel(nn.Module):
    ...
    def __init__(self, **args):
        ...
        imagebind_ckpt = torch.load(
            imagebind_ckpt_path, map_location=torch.device("cpu")
        )
    ...
    def encode_thermal(self, thermal_paths):
        inputs = {
            ModalityType.THERMAL: data.load_and_transform_thermal_data(
    ...
    def encode_image_for_web_demo(self, image_paths):
        inputs = {
            ModalityType.VISION: data.load_and_transform_vision_data_for_web_demo(
                image_paths, self.device
            )
        }
        # convert into visual dtype
    ...
    def encode_image_for_one_shot_from_tensor(self, image_tensors):
        ...
        inputs = {
            ModalityType.VISION: torch.stack(image_tensors, dim=0).to(self.device)
        }
        # convert into visual dtype
        inputs = {key: inputs[key].to(self.llama_model.dtype) for key in inputs}
        with torch.no_grad():
    ...
    def encode_image_for_one_shot_with_aug(self, image_paths):
        image_tensors = data.load_and_transform_vision_data(
            image_paths, self.device
        ...
        rotated_images = (
            torch.zeros((4, B, C, H, W)).to(self.llama_model.dtype).to(self.device)
        )
    ...
    def prompt_wrap(
        self, img_embeds, input_ids, target_ids, attention_mask, anomaly_embedding=None
    ):
        """
        ...
        if anomaly_embedding != None:
            ...
            empty_targets = (
                torch.ones(
                    [
                        batch_size,
                        1
                        + p_before_embeds.size()[1]
                        + 1
                        + p_middle_embeds.size()[1]
                        + anomaly_embedding.size()[1],
            ...
            empty_targets = (
                torch.ones(
                    [
                        batch_size,
                        1 + p_before_embeds.size()[1] + 1 + p_middle_embeds.size()[1],
                    ],  # 1 (bos) + s1 + 1 (image vector)
                    dtype=torch.long,
                )
                .to(self.device)
    ...
    def forward(self, inputs):
        ...
        if "masks" in inputs:
            ...
            feats_text_tensor = encode_text_with_prompt_ensemble(
                self.visual_encoder, class_name, self.device
            )
            ...
            for layer in range(len(patch_tokens)):
                ...
                anomaly_map = (
                    100.0 * patch_tokens[layer] @ feats_text_tensor.transpose(-2, -1)
                )
            ...
            anomaly_map_all = torch.mean(
                torch.stack(anomaly_maps, dim=0), dim=0
            ).unsqueeze(1)

            if random.randint(0, 1) == 0 and len(inputs["img_paths"]) == len(
                image_paths
            ):
                ...
                for path in inputs["img_paths"]:
                    if "all_anomalygpt" not in path and "visa" in path.lower():
                        ...
                        normal_path = find_first_file_in_directory(
                            "/".join(normal_path.split("/")[:-1])
                        )
                    else:
                        normal_path = path.replace("test", "train")
                        normal_path = find_first_file_in_directory(
                            "/".join(normal_path.split("/")[:-2]) + "/good"
                        )
                    normal_paths.append(normal_path)

                print(normal_paths)
                query_patch_tokens = self.encode_image_for_one_shot_from_tensor(
                    image_paths
                )
                normal_patch_tokens = self.encode_image_for_one_shot_with_aug(
                    normal_paths
                )
                sims = []
                B = len(image_paths)
                ...
                for i in range(len(query_patch_tokens)):
                    query_patch_tokens_reshaped = query_patch_tokens[i].view(
                        B, 256, 1, 1280
                    )
                    normal_tokens_reshaped = normal_patch_tokens[i].reshape(
                        B, 1, -1, 1280
                    )
                    cosine_similarity_matrix = F.cosine_similarity(
                        query_patch_tokens_reshaped, normal_tokens_reshaped, dim=-1
                    )
            ...
            inputs_embeds, targets, attention_mask = self.prompt_wrap(
                img_embeds, input_ids, target_ids, attention_mask, anomaly_map_prompts
            )
    ...
    def extract_multimodal_feature(self, inputs, web_demo):
        ...
        if inputs["image_paths"]:
            ...
            if not web_demo:
                ...
                feats_text_tensor = encode_text_with_prompt_ensemble(
                    self.visual_encoder, [c_name], self.device
                )
            else:
                image_embeds, _, patch_tokens = self.encode_image_for_web_demo(
                    inputs["image_paths"]
                )
                feats_text_tensor = encode_text_with_prompt_ensemble(
                    self.visual_encoder, [c_name], self.device
            ...
            if inputs["normal_img_paths"]:
                query_patch_tokens = self.encode_image_for_one_shot(
                    inputs["image_paths"]
                )
                if "mvtec" in "normal_img_paths":
                    normal_patch_tokens = self.encode_image_for_one_shot_with_aug(
                        inputs["normal_img_paths"]
                    )
                else:
                    normal_patch_tokens = self.encode_image_for_one_shot(
                        inputs["normal_img_paths"]
                    )
                sims = []
                ...
                for i in range(len(query_patch_tokens)):
                    ...
                    cosine_similarity_matrix = F.cosine_similarity(
                        query_patch_tokens_reshaped, normal_tokens_reshaped, dim=2
                    )
                    sim_max, _ = torch.max(cosine_similarity_matrix, dim=1)
                    sims.append(sim_max)
    ...
    def generate(self, inputs, web_demo=False):
        ...
        output_text = self.llama_tokenizer.decode(
            outputs[0][:-2], skip_special_tokens=True
        )
